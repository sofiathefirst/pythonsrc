...
import select
import sys
# sys.path.append('/home/pi/xsens_imu')
import mti_main
import os
import termios
import math
...
class TimerThread(threading.Thread):
    ...
    def run(self):
        ...
        print 'imu start '
        self.xsense = mti_main.XSensDriver()
        self.xsense.setDaemon(True)
        self.xsense.start()
        ...
        while (home_angle_init_cnt < 200):
            mti_main.imu_mutex.acquire()
            roll, pitch, yaw = Quaternion_2_Euler(self.xsense.w, self.xsense.x, self.xsense.y, self.xsense.z)
            home_angle =  pitch*180.0/3.14159265358979
            mti_main.imu_mutex.release()
        ...
        while self.__stopf:
            time.sleep(0.02)        #频率100Hz
            self.cur_time = time.time()

            mti_main.imu_mutex.acquire()
            roll, pitch, yaw = Quaternion_2_Euler(self.xsense.w, self.xsense.x, self.xsense.y, self.xsense.z)
            # roll = roll*180.0/3.14159265358979
            # pitch = pitch*180.0/3.14159265358979
            # yaw = yaw*180.0/3.14159265358979

            # print self.xsense.gyr_y*180.0/3.14159265358979,'hhhh'
            ...
            self.cur_home_speed = -self.xsense.gyr_y * 180 / 3.14159265358979       # 圆盘倾角角速度 单位°/s
...
#                 
            ...
            home_roll_speed = self.xsense.gyr_x * 180 / 3.14159265358979       # 圆盘滚转角速度 单位°/s
            #  侧向角速度PID代码
...
#             print "%.6f  %.6f "%(runout_speed_goal, -self.runout_goback)
            ...
            Yaw_speed_err = 0 - self.xsense.gyr_z * 180 / 3.14159265358979
...
#                 print "%s  %s  %s  %s"%(self.cur_home_speed,self.cur_home_acc,self.cur_home_acc_filter,self.cur_home_acc_kalfilter)
            ...
            ax,ay,az = acc_NED(roll, pitch, 0,\
                self.xsense.acc_x, self.xsense.acc_y, self.xsense.acc_z)   # 转化到与IMU相关的水平坐标系
            print>>f,'%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f' %(self.cur_time, ax, ay, az, \
                self.xsense.gyr_x* 180 / 3.14159265358979, self.xsense.gyr_y* 180 / 3.14159265358979, self.xsense.gyr_z* 180 / 3.14159265358979, \
                roll* 180 / 3.14159265358979, pitch* 180 / 3.14159265358979, yaw* 180 / 3.14159265358979, \
            ...
            if self.up_flag == 1:
                ...
                if up_time < 1.3:
                    self.Sx += self.Vx*delta_time + ax*delta_time*delta_time/2
...
# 四元数转换为欧拉角 单位rad/s
def Quaternion_2_Euler(w,x,y,z):
